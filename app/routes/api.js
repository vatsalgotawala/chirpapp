		...
		if (req.body.username === null || req.body.username === '' || req.body.password === null || req.body.password === '' || req.body.email === null || req.body.email === '' || req.body.name === null || req.body.name === '') {
			...
			user.save(function(err){
				if(err){
					...
					var email = {
					  	from: 'Chirp Staff, staff@Chirp.com',
					  	to: user.email,
		...
		User.findOne({username: req.body.username}).select('email username password active').exec(function(err, user){
			...
			if(!user){
				...
				if(!validPassword){
					...
					res.json({success: true, message: 'User authenticated', username: user.username, token: token});
				}
			}
		});
	});
					...
					user.save(function(err){
						if(err){
							...
							var email = {
							  	from: 'Chirp Staff, staff@Chirp.com',
							  	to: user.email,
							  	subject: 'Chirp Account Activated',
				...
				if(!req.params.email){
					...
					res.json({success: false, message: 'E-mail was not found.'});
						...
						var email = {
						  	from: 'Chirp Staff, staff@Chirp.com',
						  	to: user.email,
						  	subject: 'Chirp Username Request',
				...
				user.save(function(err){
					if(err){
						...
						var email = {
						  	from: 'Chirp Staff, staff@Chirp.com',
						  	to: user.email,
						  	subject: 'Chirp Reset Password',
						  	text: 'Hello ' + user.name + ', this e-mail is to notify you that your password has been reset successfully on Chirp.com.',
						  	html: 'Hello <strong>' + user.name + '</strong>,<br><br>This e-mail is to notify you that your password has been reset successfully on Chirp.com.'
						};
			...
			if(!mainUser){
				...
				if(mainUser.permission !== 'admin'){
					...
					Post.remove({_id: req.params.id}, function(err) {
						if (err){
							res.json({success: false, message: 'Error while deleting post.'});
						}
						else{
				...
				user.save(function(err){
					if(err){
						...
						var email = {
						  	from: 'Chirp Staff, staff@Chirp.com',
						  	to: user.email,
						  	subject: 'Chirp Change Password',
						  	text: 'Hello ' + user.name + ', this e-mail is to notify you that your password has been changed successfully on Chirp.com.',
						  	html: 'Hello <strong>' + user.name + '</strong>,<br><br>This e-mail is to notify you that your password has been changed successfully on Chirp.com.'
						};
